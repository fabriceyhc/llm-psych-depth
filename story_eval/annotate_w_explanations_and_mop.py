...
def annotate_psd(lm, persona, story):
    ...
    with assistant():
        ...
        Emotion Provoking: {gen('emotion_provoking_explanation', stop='.')}.
...
for model_id in model_ids:
    ...
    for participant_id, persona in enumerate(personas):
        ...
        for index, row in dataset.iterrows():
            ...
            if not existing_annotations.empty and ((existing_annotations["participant_id"] == row["participant_id"]) & (existing_annotations["story_id"] == row["story_id"]) & (existing_annotations["premise_id"] == row["premise_id"])).any():
                print(f"Skipping already annotated row: participant_id={row['participant_id']}, story_id={row['story_id']}, premise_id={row['premise_id']}")
                continue
